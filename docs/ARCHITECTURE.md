...
| 路径 | 职责 |
| --- | --- |
| `src/main.ts` | 输入状态机、清空后再 Enter 复制、主题应用、变量/历史渲染、窗口自适应、剪贴板与焦点事件 |
| `src/brackets.ts` | 成对括号插入、跳过已有右括号、末尾括号补齐 |
| `src/completion.ts` | 带简短说明的变量、斜杠命令和点操作候选匹配及文本替换范围计算 |
| `src/submission.ts` | 将输入解析为无操作、复制、指令或表达式提交动作 |
| `src/input-normalization.ts` | 中文/全角数字、括号、标点和数学符号的前端即时转换 |
| `src/i18n.ts` | 系统语言解析、前端三语词典、参数插值和英语回退 |
| `src/commands.ts` | 斜杠命令注册、解析、帮助、`/clean`、`/color` 与 `/plugin` 管理命令 |
...
7. 数值结果更新变量、类型元数据与 `res`；纯文本转换不覆盖 `res`。两类结果都写入历史并裁剪到 100 条。
8. 同步持久化成功后返回 IPC 响应。
...
- 主窗口固定宽度；变量或命令内容溢出时，前端用四次缓出曲线向下增高窗口，并受当前显示器工作区与 860px 上限约束。
- 外壳使用直角裁切，避免 Windows 10 无边框窗口的圆角合成瑕疵。
- 发布阶段开启 Rust release 优化、LTO 和符号裁剪。

## 7. 安全边界

- CSP 默认只允许本地资源。
- Tauri capability 开放写剪贴板，以及窗口自适应所需的尺寸、位置、缩放比例和显示器只读查询与 `setSize` 权限。
- 文件系统只由 Rust 访问，前端不能传入任意路径。
- 表达式语言不包含文件、网络、进程或动态代码能力。
- 用户变量只保存 `String -> f64` 及受限的值类型元数据，不保存可执行内容。
- v0.2 插件接口不包含磁盘或网络加载器；只有宿主显式提供的可信插件对象才能激活。

## 8. 已知框架限制

- 当前容器可验证 TypeScript 构建与前端测试，但正式桌面构建仍需 Rust 工具链和各平台系统依赖。
- macOS/Linux 的全局快捷键与自启动需要真实平台回归；Wayland 可能要求桌面门户或用户授权。
- v0.2 提供变量只读列表和命令式历史/主题管理，但尚无快捷键编辑或完整设置界面。
